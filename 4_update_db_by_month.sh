...
get_previous_month() {
    ...
    local year
    year=$(echo "$previous_month" | cut -d'-' -f1)
    local month
    month=$(echo "$previous_month" | cut -d'-' -f2)
    echo "$year $month"
}
...
run_update() {
    ...
    python3 "$ROOT_PATH/3_update_by_month.py" --year "$year" --month "$month" \
        > "$ROOT_PATH/month_update.log" 2>&1
}

export -f run_update

if [ $# -eq 0 ]; then
    read -r year month <<< "$(get_previous_month)"
